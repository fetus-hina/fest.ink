...
class m151024_070218_6th_data extends Migration
{
    ...
    public function safeUp()
    {
        ...
        foreach ($this->findAllJsonFiles() as $entry) {
            ...
            $mvpList = [];
            foreach ($parsed->getMvpList() as $mvp) {
                $mvpList[] = [
                    'data_id' => $id,
                    'color_id' => $mvp['x_win_team_side'] === 'alpha' ? 1 : 2,
                    'name' => $mvp['win_team_mvp'],
                ];
            }
            $this->batchInsert('mvp', ['data_id', 'color_id', 'name'], $mvpList);
        }
    }

    public function safeDown()
    {
        // DELETE official_win_data/mvp
        ...
        $sql = "DELETE FROM {{mvp}} WHERE {{mvp}}.[[data_id]] IN ( {$inner} )";
        $this->execute($sql, [':id' => $this->getFestId()]);
